...
const FloatingForm: React.FC<FloatingFormProps> = ({
  ...
  useEffect(() => {
    ...
    setFormStatus(FormStatus.EDITING);
  }, [previousStatus]);
  ...
  const enableForm = [FormStatus.EDITING, FormStatus.IDLE].includes(formStatus);
  ...
  return (
    <>
      ...
      {!enableForm && (
        <div className="absolute h-full top-0 w-full z-20 bg-black bg-opacity-25 flex items-center justify-center">
          <Loading />
        </div>
      )}
    </>
